...
static void homeaxis(const AxisEnum axis) {
  ...
  #if ENABLED(Z_DUAL_ENDSTOPS)	// false
    if (axis == Z_AXIS) {
...
inline void gcode_G0_G1(
  ...
  #if ENABLED(NO_MOTION_BEFORE_HOMING)	// false
...
//SERIAL_ECHOLNPGM("****gcode_G28****");
  ...
  endstops.enable(true); // Enable endstops for next homing move
    ...
    #if Z_HOME_DIR > 0  // If homing away from BED do Z first 	// false 
    ...
    #if ENABLED(QUICK_HOME)	// false 
      if (home_all || (homeX && homeY)) quick_home_xy();

    #endif

    #if ENABLED(HOME_Y_BEFORE_X)	 // false 
      // Home Y
      if (home_all || homeY) {
        HOMEAXIS(Y);
        #if ENABLED(DEBUG_LEVELING_FEATURE)
    ...
    // Home X
    if (home_all || homeX) {  	// true
      #if ENABLED(DUAL_X_CARRIAGE)	//false
...
//SERIAL_CHAR("**********gcode_G29***********\n");
  
    if(AutoLevelStatus &&PrinterStatusKey[0] == 1 ) return;//turn off Autolevel
	...
	// SERIAL_ECHO(measured_z);
            if (isnan(measured_z)) {
              ...
              z_values[xCount][yCount] = measured_z + zoffset;
		//eeprom_write_byte((unsigned char*)AutoLeve_EEPROM+4*showcount, z_values[xCount][yCount]);
		/*	
		SERIAL_CHAR(" **z_values[");
		SERIAL_ECHO((int)xCount);
		SERIAL_CHAR("][");
		SERIAL_ECHO((int)yCount);
		SERIAL_CHAR("]== ");
		SERIAL_ECHO(z_values[xCount][yCount]);
		SERIAL_CHAR("  \n");
		*/
...
inline void gcode_M109() 
{
  ...
  do 
  {
	...
	if (wait_for_heatup)
	{
		if(PreheatStatus[0])
		{
			...
			if(PrintModeTime == 1) PrintModeTime = print_job_timer.duration();
			else PrintModeTime = 0;
  ...
  inline void gcode_M190() 
  {
   ...
   //SERIAL_ECHOLN(" ***gcode_M190*** ");
	...
	//SERIAL_ECHOLN(" ** wait_for_heatup==  ");
	//SERIAL_ECHO(wait_for_heatup?1:0);
    if (wait_for_heatup)
    {
	if(PreheatStatus[1])
	{
...
void set_current_from_steppers_for_axis(const AxisEnum axis) {
  ...
  void bilinear_line_to_destination(float fr_mm_s, uint16_t x_splits = 0xFFFF, uint16_t y_splits = 0xFFFF) {
    ...
    if (cx2 != cx1 && TEST(x_splits, gcx)) {
      ...
      // Already split on a border
      line_to_destination(fr_mm_s);	// here
      set_current_to_destination();
      return;
    }
  ...
  inline bool prepare_move_to_destination_cartesian() {
      ...
      if (current_position[X_AXIS] == destination[X_AXIS] && current_position[Y_AXIS] == destination[Y_AXIS])
        ...
        #elif ENABLED(AUTO_BED_LEVELING_BILINEAR)	// true
          if (planner.abl_enabled && !AutoLevelStatus) { // direct use of abl_enabled for speed
            bilinear_line_to_destination(fr_scaled);	// here
            return true;
          }
          else
        #endif
...
void idle(
  ...
  #if ENABLED(I2C_POSITION_ENCODERS)		// 0
